...
vim.keymap.set({'n', 'i'}, '<C-h>',
               [[<cmd>lua require("tmux").move_left()<cr>]], opts) -- Move cursor to the left window
vim.keymap.set({'n', 'i'}, '<C-j>',
               [[<cmd>lua require("tmux").move_bottom()<cr>]], opts) -- Move cursor to the down window
vim.keymap.set({'n', 'i'}, '<C-k>', [[<cmd>lua require("tmux").move_top()<cr>]],
               opts) -- Move cursor to the up window
vim.keymap.set({'n', 'i'}, '<C-l>',
               [[<cmd>lua require("tmux").move_right()<cr>]], opts) -- Move cursor to the right window
...
vim.keymap.set({'n'}, '<leader>tv', '<cmd>:botright vnew <BAR> :terminal<CR>',
               opts) -- Opens terminal on the right side
vim.keymap.set({'n'}, '<leader>th', '<cmd>:botright new <BAR> :terminal<CR>',
               opts) -- Opens terminal on the bottom side
...
vim.keymap.set("n", ';;',
               [[<cmd>lua require("telescope.builtin").resume()<CR>]], opts) -- Recuperate last search
vim.keymap.set('n', '<Space>fd',
               [[<cmd>lua require("telescope.builtin").diagnostics()<CR>]], opts) -- Displays all files in the document
vim.keymap.set("n", "<Space>fg",
               [[<cmd>lua require('telescope').extensions.live_grep_args.live_grep_args()<CR>]],
               opts) -- Displays all files in the document
...
M.oil_select_vertical = {
    {'n'}, '<Space>ev', ':Oil actions.select { vertical = true }<CR>', opts
} -- Open in vertical split
M.oil_select_horizontal = {
    {'n'}, '<Space>es', ':Oil actions.select { horizontal = true }<CR>', opts
} -- Open in horizontal split
M.oil_select_tab = {
    {'n'}, '<Space>et', ':Oil actions.select { tab = true }<CR>', opts
} -- Open in new tab
...
M.oil_cd_tab = {
    {'n'}, '<Space>e~', ':Oil actions.cd { scope = "tab" }<CR>', opts
} -- Change directory for tab
M.oil_change_sort = {{'n'}, '<Space>es', ':Oil actions.change_sort<CR>', opts} -- Change sort
M.oil_open_external = {
    {'n'}, '<Space>ex', ':Oil actions.open_external<CR>', opts
} -- Open externally
M.oil_toggle_hidden = {
    {'n'}, '<Space>e.', ':Oil actions.toggle_hidden<CR>', opts
} -- Toggle hidden files
M.oil_toggle_trash = {
    {'n'}, '<Space>e\\', ':Oil actions.toggle_trash<CR>', opts
} -- Toggle trash

-------------- Keymaps form git commands ----------------
vim.keymap.set({'n'}, '<Space>ggg',
               '<cmd>Gitsigns toggle_current_line_blame<CR>', opts) -- Displays inline changes made to the file.
...
vim.api
    .nvim_set_keymap('n', 'gd', '<cmd>lua vim.lsp.buf.definition()<CR>', opts) -- Go to method definition
vim.api.nvim_set_keymap('n', 'gD', '<cmd>lua vim.lsp.buf.declaration()<CR>',
                        opts) -- Go to method declaration
vim.api.nvim_set_keymap('n', 'gi', '<cmd>lua vim.lsp.buf.implementation()<CR>',
                        opts) -- Go to method implementation
vim.api
    .nvim_set_keymap('n', 'gr', '<cmd>lua vim.lsp.buf.references()<CR>', opts) -- Go to method references
vim.keymap.set({'n', 'i'}, '<C-RightMouse>',
               '<cmd>lua vim.lsp.buf.definition()<CR>', opts) -- Go to method definition
...
end, opts) -- Show code information
