_G.KEYMAPS = {
    ...
    open_rigth_terminal = { { 'n' }, '<leader>tr', '<cmd>:botright vnew <BAR> :terminal<CR>' },
    -- Opens terminal on the bottom side
    open_bottom_terminal = { { 'n' }, '<leader>tb', '<cmd>:botright new <BAR> :terminal<CR>' },

    ------------------------------------> KEYBOARD SHORTCUTS FOR FILE MANAGER <---------------------------------------------------

    -- Opens file tree
    open_file_manager = { { 'n', 'i' }, '<C-b>', '<Esc><cmd>:NeoTreeFocusToggle<CR>' },
    -- Displays all files in the document
    telescope_files = { { 'n' }, '<leader>ff', '<cmd>Telescope fd<CR>' },
    -- Displays all alerts found in the buffer
    telescope_diagnostics = { { 'n' }, '<leader>fe', '<cmd>Telescope diagnostics<CR>' },
    ...
    telescope_branches = { { 'n' }, '<leader>gb', '<cmd>Telescope git_branches<CR>' },
    ...
    gitsigns_toggle_line_blame = { { 'n' }, '<C-g>', '<cmd>:Gitsigns toggle_current_line_blame<CR>' },
    ...
    show_diagnostic = { { 'n' }, '<C-S-M>',
        function() vim.diagnostic.open_float(0, { scope = "line", border = "rounded" }) end },
    show_diagnostic_secondary = { { 'n' }, '<C-e>',
        function() vim.diagnostic.open_float(0, { scope = "line", border = "rounded" }) end },
    -- LSP signature
